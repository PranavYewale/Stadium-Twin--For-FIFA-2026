...
def run_simulation_step(app, socketio, tick_count):
...
def run_simulation_loop(app, socketio):
    ...
    while True:
        try:
            time.sleep(3.0)
            GLOBAL_TICK_COUNT += 1
            run_simulation_step(app, socketio, GLOBAL_TICK_COUNT)
...
def trigger_lazy_tick(app, socketio=None):
    """
    Triggers a lazy simulation tick if the last tick was more than 3 seconds ago.
    This is extremely useful on platforms like PythonAnywhere where background threads do not run.
    """
    global LAST_TICK_TIME, GLOBAL_TICK_COUNT
    now = time.time()
    # If the simulation hasn't ticked for 3.5 seconds, tick it now inline
    if LAST_TICK_TIME == 0.0 or (now - LAST_TICK_TIME) > 3.5:
        GLOBAL_TICK_COUNT += 1
        try:
            run_simulation_step(app, socketio, GLOBAL_TICK_COUNT)
            return True
        except Exception as e:
            print(f"Lazy tick failed: {e}")
    return False
